...
function borders(input: string[], threeLetterCodes: Map<string, string>) {
    ...
    return input.map((code: string) => (
        <Link
            key={code}
            href={`/${threeLetterCodes.get(code)?.replaceAll(" ", "%20")}`}
        >
            {threeLetterCodes.get(`${code}`)}
        </Link>
    ))
}
...
export default async function Page(country: { params: { detail: string } }) {
    const info = await fetch(
        `https://restcountries.com/v3.1/name/${country.params.detail}?fields=name,flags,population,region,capital,subregion,tld,currencies,languages,borders`,
    )
    ...
    let index: number = jsoninfo.findIndex(
        (nation: { name: { common: string } }) =>
            nation.name.common.toUpperCase() ==
            `${country.params.detail.toUpperCase()}`,
    )
    if (index == -1) {
        index = 0
    }
    ...
    const countryInfo: Nation = {
        ...
        localName: Object.values(currentNation.name.nativeName) as {
            common: string
        }[],
        ...
        languages: Object.values(currentNation.languages) as string[],
    ...
    const countriesfull = (await countries.json()) as {
        name: { common: string }
        cca3: string
    }[]
            ...
            <main className={styles.main}>
                ...
                <div className={`${styles.grid}`}>
                    ...
                    <div>
                        ...
                        <div className={`${styles.flexRow} ${styles.details}`}>
                            <div>
                                <p className={styles.info}>
                                    Native Name:{" "}
                                    <Tooltip
                                        text={`${countryInfo.localName[0].common}`}
                                        tooltipItems={
                                            countryInfo.localName.map(
                                                (obj) => obj.common,
                                            ) as string[]
                                        }
                                    />
                            ...
                            <div>
                                ...
                                <p className={styles.info}>
                                    Currencies:{" "}
                                    <Tooltip
                                        text={`${countryInfo.currency[0].name}`}
                                        tooltipItems={
                                            countryInfo.currency.map(
                                                (obj) => obj.name,
                                            ) as string[]
                                        }
                                    />
                                ...
                                <p className={styles.info}>
                                    Languages:{" "}
                                    <Tooltip
                                        text={`${Object.values(countryInfo.languages)[0]}`}
                                        tooltipItems={Object.values(
                        ...
                        <div>
                            <p className={`${styles.borders} ${styles.info}`}>
                                Border Countries:{" "}
                                {borders(countryInfo.borders, threeLetterCodes)}
                            </p>
                        </div>
                    </div>
                </div>
